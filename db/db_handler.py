...
def fetch_restaurant_election_options(election_id):
    query = "SELECT e.start_time, r.res_id, r.res_name, r.res_loc " \
            "FROM elections e " \
...
def fetch_lastest_election_data():
    q_votes =   "SELECT u.user_name, r.res_name " \
                "FROM users u " \
    ...
    q_total =   "SELECT r.res_name, COALESCE(COUNT(v.restaurant_id), 0) AS total_votes " \
...
def fetch_n_unknown_random_restaurants(nr):
    query = "SELECT res_id, re.res_name, re.res_loc " \
            "FROM restaurants re " \
...
def fetch_restaurants_avg():
    ...
    query = "SELECT re.res_name, count(rv.*) review_count, " \
